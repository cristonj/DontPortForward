...
export default function ActiveCommandCard({ log, isExpanded, onToggle, onKill, onDelete, getLastLines, isOptimistic = false }: ActiveCommandCardProps) {
  return (
    <div className={`active-command-card rounded-xl p-4 shadow-xl backdrop-blur-sm relative group overflow-hidden ${isOptimistic ? 'opacity-80' : ''}`}>
      ...
      >
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <div className="text-xs text-gray-500 font-mono flex items-center gap-3 flex-wrap">
            {!isOptimistic && <CommandIdBadge id={log.id} />}
            <span className="inline-flex items-center gap-1.5">
              <PulsingDot size="sm" color={isOptimistic ? "warning" : "accent"} />
              <span className={`uppercase font-semibold tracking-wide ${isOptimistic ? 'text-yellow-400' : 'text-terminal-accent'}`}>
                {isOptimistic ? 'sending' : log.status}
              </span>
            </span>
          </div>
        </div>
        ...
        <div className="ml-4 flex items-center gap-2 shrink-0 mt-0 z-50">
          {isOptimistic ? (
            <span className="px-3 py-1.5 text-yellow-400/70 text-xs font-medium flex items-center gap-2">
          ...
          <button
            ...
            title={isOptimistic ? "Cancel sending" : "Delete task"}
          >
              ...
              <div className="flex items-center justify-center h-28">
                <div className="text-center">
                  <div className="mb-3 flex justify-center">
                    <PulsingDot size="lg" color={isOptimistic ? "warning" : "accent"} />
                  </div>
                  <div className="text-gray-500 text-xs">
                    {isOptimistic ? "Sending command to device..." : "Waiting for output..."}
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="font-mono text-xs text-gray-500 pl-3 border-l-2 border-gray-700 line-clamp-1">
            {log.output ? (
              ...
              <span className="text-gray-600 italic flex items-center gap-2">
                <PulsingDot size="sm" color={isOptimistic ? "warning" : "accent"} />
                {isOptimistic ? "Sending to device..." : "Waiting for output..."}
              </span>
            )}
          </div>
        )}
