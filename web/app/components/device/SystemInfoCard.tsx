...
const InfoRow = memo(function InfoRow({ label, value, mono = false, truncate = false, isLast = false }: InfoRowProps) {
  return (
    <div className={`flex justify-between items-center py-2 ${!isLast ? 'border-b border-gray-800/50' : ''}`}>
      <dt className="text-gray-500">{label}</dt>
      <dd 
        className={`text-gray-200 font-medium text-right ml-4 ${mono ? 'font-mono' : ''} ${truncate ? 'truncate max-w-[200px]' : ''}`}
